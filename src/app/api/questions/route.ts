...
export const POST = async (req: Request) => {
    try {
        ...
        const {topic, amount, type, difficulty} = quizCreationSchema.parse(body);
        ...
        if (type === "open_ended") {
            questions = await gemini_output(
                ...
                new Array(amount).fill(`You need to generate a random ${difficulty} open-ended questions about ${topic} and total of ${amount} questions only`),
                {
                question: "question",
            ...
            questions = await gemini_output(
                ...
                `You need to generate random ${difficulty} mcq question about ${topic} and total of ${amount} questions only`
                ),
                {
                question: "question",
