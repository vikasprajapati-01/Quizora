...
function OpenEndedPercentage({ percentage }: { percentage: number }) {
  ...
  return (
    <div className="bg-[#0a0a0a] border border-gray-700 rounded-lg p-3 flex items-center gap-3">
      <FaThLarge className="text-[#ff7f01] w-6 h-6" />
      <div className="flex items-center">
        <span className="text-2xl font-bold text-white">{isNaN(pct) ? 0 : pct}</span>
        <FaPercent className="text-gray-400 w-4 h-4 ml-1" />
      </div>
    </div>
  );
}
...
function BlankAnswerInput({ 
  ...
  // Move the setState to useEffect - THIS IS THE FIX
  React.useEffect(() => {
    setBlankAnswer(answerWithBlanks);
  }, [answerWithBlanks, setBlankAnswer]);

  return (
    <div className="w-full bg-[#0a0a0a] border border-gray-700 rounded-lg p-4 mb-4">
      <h3 className="text-sm text-gray-400 mb-3">Fill in the blanks:</h3>
          ...
          return (
            <React.Fragment key={index}>
              ...
              {index === answerWithBlanks.split(blank).length - 1 ? (
                ...
                <input
                  id="user-blank-input"
                  className="mx-2 px-2 py-1 text-center border-b-2 border-[#ff7f01] bg-transparent text-white w-24 focus:border-b-4 focus:outline-none focus:bg-[#ff7f01]/10 transition-all"
                  type="text"
                  placeholder="___"
                />
              )}
...
const OpenEnded: React.FC<Props> = ({ game }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12">
      ...
      <div className="w-full max-w-4xl bg-[#171717] shadow-lg rounded-lg p-6 border border-gray-800 relative">
        {/* Loading overlay */}
        {isChecking && (
